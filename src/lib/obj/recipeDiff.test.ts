...
function node(partial: Partial<RecipeNode> & { id: string }): RecipeNode {
	return {
		id: partial.id,
...
describe('formatIngredientChange', () => {
	...
	it('formats an edit with before → after when target exists in prior state', () => {
		...
		expect(formatIngredientChange(change, prior)).toBe('EDIT 1 tbsp Butter → 2 tbsp Butter');
	});
...
describe('formatDirectionChange', () => {
	it('formats an add with quoted body', () => {
		...
		expect(formatDirectionChange(change, { ingredients: new Map(), directions: new Map() })).toBe(
	...
	it('formats an edit with before → after when target exists', () => {
		...
		const prior: RecipeStateMaps = {
			ingredients: new Map(),
			directions: new Map([['d1', direction({ id: 'd1', body: 'Mix flour and water.' })]])
		};
	...
	it('formats a remove showing what was removed', () => {
		...
		expect(formatDirectionChange(change, prior)).toBe('REMOVE "Mix flour and water."');
	});
});
...
describe('formatChain', () => {
	...
	it('handles ingredient and direction changes in the same node', () => {
		...
		expect(entries[1].changes[0]).toMatchObject({
			kind: 'ingredient',
			text: 'EDIT 200 g Flour → 250 g Flour'
		});
		expect(entries[1].changes[1]).toMatchObject({
			kind: 'direction',
			text: 'EDIT "Mix flour." → "Whisk flour."'
		});
	});
	...
	it('threads through the change note so the UI can show a note icon', () => {
		...
		const entries = formatChain(nodes);
		expect(entries[1].changes[0].note).toBe(
			'substituted because the shop was out of salted butter'
		);
	...
	it('formats the French-omelette chain end-to-end', () => {
		...
		expect(second).toContain(
			'EDIT "Melt butter in a pan over medium heat." → "Melt butter over low heat until foamy."'
		);
		expect(second).toContain(
			'EDIT "Pour in eggs, cook, stirring gently until set." → "Pour in eggs and stir constantly with the flat of a fork, keeping the curds moving. Do not brown."'
		);
	});
});
